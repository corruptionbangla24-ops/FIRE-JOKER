...
// 🔥 ৩-রিলস ওরিজিনাল ফায়ার জোকার ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const fireJokerPool = ["FIRE_JOKER", "GOLDEN_BAR", "LUCKY_7", "STAR_NEON", "PLUM_FRUIT", "CHERRY_BAR", "X_SYMBOL"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/joker-balance', async (req, res) => {
...
// 🛫 ২. ফায়ার জোকার ৩-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/joker-spin', async (req, res) => {
    ...
    if (reqAmount < 1 || reqAmount > 20000) {
        return res.json({ success: false, message: "🚨 Invalid Bet Amount (৳১ - ৳২০০০0)" });
    }
    ...
    try {
        ...
        // 🔒 [ইনসাফিসিয়েন্ট প্রোটেকশন বর্ম]: অ্যাকাউন্টে টাকা কম থাকলে বা জিরো ব্যালেন্স হলে বাজি রিফিউজд ভাই ভাই!
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.joker_target) ? balResponse.data.joker_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৩-রিলস ফায়ার জোকার গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            loopSafety++;
            
            // ৩টি প্রিমিয়াম রিলের র্যান্ডম স্টপ প্রতীক জেনারেটর ভাই ভাই
            spinResults = [];
            for (let r = 0; r < 3; r++) {
                spinResults.push(fireJokerPool[Math.floor(Math.random() * fireJokerPool.length)]);
            }
            ...
            if (maxSameSymbolsCount === 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওッズ]: ৩টি ফায়ার জোকার মিললে সর্বোচ্চ ৮০০ গুণের গ্র্যান্ড ফায়ার জ্যাকপট!
                if (spinResults[0] === "FIRE_JOKER" && spinResults[1] === "FIRE_JOKER" && spinResults[2] === "FIRE_JOKER") {
                    winMultiplier = 800.00;
                } else if (maxSameSymbolsCount === 3) {
                    winMultiplier = 80.00; // ৩টি অন্য সেম মিললে ৮০ গুণ প্রফিট চেইন
                }
            } else if (maxSameSymbolsCount === 2) {
                finalStatus = "win";
                winMultiplier = 4.00; // ২টি মিললে ৪ গুণ আংশিক রিটার্ন ব্যাকআপ
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_jackpot" && winMultiplier === 800.00) isLoopActive = false;
            } else {
                // ৮০০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.০০৫% এ লক ভাই ভাই
                if (winMultiplier === 800.00 && Math.random() > 0.0005) continue;
        ...
        console.error("Fire Joker Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Fire Joker 3-Reel Slot Engine!"); });

// ১৯ নম্বর ফায়ার জোকার গেম নিজস্ব কাস্টম ৫১৯০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 5190; 
server.listen(PORT, () => { console.log(`🎡 Royal Fire Joker Engine Running on port ${PORT}`); });
